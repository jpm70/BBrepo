...
head
  meta charset=UTF-8
  titleVerificación de Banderas - Los Pollos Hermanostitle
  style
    body {
...
body
  ...
  div class=formulario
    form action=verificarbandera.php method=POST
      label for=numeroNúmero de Banderalabel
      select id=numero name=numero required
        option value=Selecciona una banderaoption
        php
        for ($i = 1; $i = 15; $i++) {
          echo option value='$i'Bandera $ioption;
        }
        
      select

      label for=respuestaValor de la bandera (entre ' y ')label
